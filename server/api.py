...
try:
    from voice.asr_service import asr_service_instance as _ASR
except Exception:  # noqa: BLE001
    _ASR = None

try:
    from voice.tts_service import tts_service_instance as _TTS
except Exception:  # noqa: BLE001
    _TTS = None
    ...
    try:
        while True:
            ...
            if msg_type in ["audio", "audio_partial"]:
                if _ASR is None:
                    await websocket.send_json(
                        {"type": "error", "error": "Voice (ASR) is not enabled in this deployment."}
                    )
                    continue
                ...
                try:
                    ...
                    transcription = await asyncio.to_thread(_ASR.transcribe_audio, temp_audio_path)
            ...
            try:
                ...
                # TTS generation
                if full_response.strip() and _TTS is not None:
                    try:
                        logger.info("[TTS] Generating audio for response...")
                        tts_start = time.time()
                        audio_base64 = await asyncio.to_thread(_TTS.generate_speech, full_response)
